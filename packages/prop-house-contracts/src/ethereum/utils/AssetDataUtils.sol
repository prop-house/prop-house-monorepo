...
library AssetDataUtils {
    ...
    /// @notice Decodes an asset data and returns the contract address and asset ID
    /// @param assetData The asset data describing an asset
    function decodeAssetData(bytes memory assetData) internal pure returns (address, bytes32) {
        bytes4 assetType = extractAssetType(assetData);
        if (assetType == IAssetData.ETH.selector) {
            return decodeETHAssetData(assetType, assetData);
        }
        if (assetType == IAssetData.ERC20Token.selector) {
            return decodeERC20AssetData(assetType, assetData);
        }
        if (assetType == IAssetData.ERC721Token.selector) {
            return decodeERC721AssetData(assetType, assetData);
        }
        if (assetType == IAssetData.ERC1155Token.selector) {
            return decodeERC1155AssetData(assetType, assetData);
        }
    ...
    function decodeETHAssetData(bytes4, bytes memory) private pure returns (address, bytes32) {
    ...
    function decodeERC20AssetData(bytes4 assetType, bytes memory assetData) private pure returns (address, bytes32) {
    ...
    function decodeERC721AssetData(bytes4 assetType, bytes memory assetData) private pure returns (address, bytes32) {
        address tokenAddress = readAddress(assetData, 16);
        uint256 tokenId = readUint256(assetData, 36);
        return (tokenAddress, getAssetId(assetType, tokenAddress, tokenId));
    }
    ...
    function decodeERC1155AssetData(bytes4 assetType, bytes memory assetData) private pure returns (address, bytes32) {
        address tokenAddress = readAddress(assetData, 16);
        uint256 tokenId = readUint256(assetData, 36);
        return (tokenAddress, getAssetId(assetType, tokenAddress, tokenId));
    }
