...
describe('Merkle Whitelist', () => {
  ...
  before(async () => {
    ...
    tree = new MerklePedersen(leaves);
  });
  ...
  it('returns the voting power for everyone in the list', async () => {
    ...
    expect(
      new utils.splitUint256.SplitUint256(
        `0x${vp1.low.toString(16)}`,
        `0x${vp1.high.toString(16)}`,
      ),
    ).to.deep.equal(power1);
    expect(
      new utils.splitUint256.SplitUint256(
        `0x${vp2.low.toString(16)}`,
        `0x${vp2.high.toString(16)}`,
      ),
    ).to.deep.equal(power2);
    expect(
      new utils.splitUint256.SplitUint256(
        `0x${vp3.low.toString(16)}`,
        `0x${vp3.high.toString(16)}`,
      ),
    ).to.deep.equal(power3);
    expect(
      new utils.splitUint256.SplitUint256(
        `0x${vp4.low.toString(16)}`,
        `0x${vp4.high.toString(16)}`,
      ),
  ...
  it('Should fail if an invalid proof is supplied', async () => {
    const leaf1 = merkleData.find(leaf => leaf[1] == address1)!;
    try {
      await whitelist.call('get_voting_power', {
        timestamp: BigInt(0),
        voter_address: { value: address2 },
        params: [tree.root],
        user_params: [
      ...
      expect(error.message).to.contain('MerkleWhitelist: Invalid proof supplied');
    }
  });
});
