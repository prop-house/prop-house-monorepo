...
// TODO: Move `Asset` out of `IHouseStrategy`
...
abstract contract AssetController {
    ...
    function _getAssetID(IHouseStrategy.Asset memory asset) internal pure returns (uint256) {
        if (asset.assetType == IHouseStrategy.AssetType.Native) {
            return uint256(asset.assetType);
        }
        if (asset.assetType == IHouseStrategy.AssetType.ERC20) {
    ...
    function _transfer(
        ...
        } else if (asset.assetType == IHouseStrategy.AssetType.ERC20) {
        ...
        } else if (asset.assetType == IHouseStrategy.AssetType.ERC721) {
