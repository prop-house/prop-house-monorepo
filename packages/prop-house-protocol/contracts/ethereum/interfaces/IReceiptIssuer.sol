...
interface IReceiptIssuer is IRound {
    ...
    function issueReceipt(address to, uint256 id, uint256 amount) external;

    /// @notice Batch issue one or more deposit receipts to the provided `to` address
    function issueReceipts(address to, uint256[] calldata ids, uint256[] calldata amounts) external;
}
