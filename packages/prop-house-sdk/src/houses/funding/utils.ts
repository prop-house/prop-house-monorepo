 ...
 */
export const getAwardsCalldata = (awards: Award[]) => {
  return awards.map(award => {
    switch (award.assetType) {
      case AssetType.ETH:
        return {
          amount: award.amount,
          assetId: encoding.getETHAssetID(),
        };
      case AssetType.ERC20:
        return {
          amount: award.amount,
          assetId: encoding.getERC20AssetID(award.address),
        };
      case AssetType.ERC721:
        return {
          amount: 1,
          assetId: encoding.getERC721AssetID(award.address, award.tokenId),
        };
      case AssetType.ERC1155:
        return {
          amount: award.amount,
          assetId: encoding.getERC1155AssetID(award.address, award.tokenId),
        };
      default:
