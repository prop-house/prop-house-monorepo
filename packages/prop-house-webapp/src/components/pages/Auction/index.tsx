...
const Auction = () => {
  ...
  const highestId = useAppSelector((state) => highestAuctionId(state.propHouse.auctions))

  return <>
  <div className={classes.auctionNav}>
    <Link to={`/auction/${id - 1}`}  className={clsx(id <= 1 ? classes.navDisabled : classes.navEnabled)}>
      Previous Auction
    </Link>
    <Link to={`/auction/${id + 1}`}  className={clsx(id >= highestId ? classes.navDisabled : classes.navEnabled)}>
      Next Auction
    </Link>
  </div>
